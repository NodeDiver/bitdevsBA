+++
title = "BitdevsBA Seminario Socratico #11"
template = "post.html"
[extra]
meetup_id = "evt_5avsKP9byc4xxcNF"
+++

### Cronograma

18:30hrs Puertas abiertas y socialización 

19:00hrs Seminario Socrático

20:30hrs Cierre y Networking

### Redes 
- [Twitter](https://x.com/BitdevsBA)
- [Signal Group](https://signal.group/#CjQKIIG3VsgXrejgIrt8M-d1fYXJTPAUtwt3GC07-siuxuH2EhBiWLhmZ4DvqWJ_6nkxnVZH)
- Nostr: npub17xlw230hgsz8yktgshnlrq670yd4uay998h4rzn7kh74elft6wwqxmre4a

### Lugar
Este evento se llevará a cabo en Café Martínez:
Av. Federico Lacroze 2899, C1426 CABA, Buenos Aires, Argentina.


Si quieres que tu espacio sea sede de BitDevsBA, escríbenos a: recruit@b4os.dev

[Cómo llegar](https://maps.app.goo.gl/KoegrbGFnR4oun1s7?g_st=ic)

### Sugerencias
- Sé cortés y amable. El objetivo es la búsqueda de una comprensión más profunda.
Respeta los diferentes pensamientos e ideas.

- Complementa con imágenes or websites las intervenciones verbales preparadas de más de 3 min, recuerda no extenderte más de 10 min, todas las presentaciones incluyendo preguntas son de 10 min, como máximo.

### Recordatorios
- Esta prohibido tomar fotos o videos durante el evento.

- Deja el espacio en las condiciones que lo encontraste.

### Qué es un seminario socrático de BitDevs?
El seminario socrático de BitDevs o Bitcoin developers es un evento mensual diseñado para promover el debate, el intercambio de información y la discusión sobre temas técnicos relacionados con Bitcoin y su ecosistema. Estos seminarios siguen un formato participativo, en el que se discuten y se hacen preguntas sobre temas recopilados por los miembros del grupo antes del evento.
Los temas provienen de varias fuentes, como Bitcoin Mailing List, Delving Bitcoin, Bitcoin Optech, pull requests en repositorios populares de proyectos como Bitcoin, entre otros.

- Los Seminarios Socráticos se celebran bajo la Regla de **[Chatham House](https://www.chathamhouse.org/about-us/chatham-house-rule)**:
- Cuando una reunión, o parte de ella, se celebra bajo la Regla de Chatham House, los participantes son libres de utilizar la información recibida, pero no se puede revelar ni la identidad ni la afiliación del orador u oradores, ni dónde se escucho la información, ni la de ningún otro participante.

Source https://bitdevs.org/running-a-great-socratic-seminar/

### Anuncios

Un agradecimiento especial a nuestro sponsor **[Libreria de Satoshi](https://libreriadesatoshi.com/)**.

### Oportunidades educativas o laborales
- [Bitcoiner Jobs](https://bitcoinerjobs.com/)
- [Summer of Bitcoin](https://www.summerofbitcoin.org/)
- [Librería de Satoshi](https://libreriadesatoshi.com/careers)
- [B4OS](https://b4os.dev)

### Estadísticas de la Red
- [mempool.space](https://mempool.space/)
- [Clark Moody Dashboard](https://dashboard.clarkmoody.com/)
- [utxo.live](https://utxo.live/)
- [bitcointreasuries.net](https://bitcointreasuries.net/)
- [non standard txs dataset](https://bitcoin-data.github.io/non-standard-transactions/)
- [mining centralization index](https://mainnet.observer/charts/mining-pools-centralization-index-with-proxy-pools/?c)

### Noticias



## Temas


### Bitcoin
 
**Releases:**
- [Bitcoin Core 31.0rc1](https://bitcoincore.org/bin/bitcoin-core-31.0/) — [Testing Guide](https://github.com/bitcoin-core/bitcoin-devwiki/wiki/31.0-Release-Candidate-Testing-Guide)
- [BDK wallet 3.0.0-rc.1](https://github.com/bitcoindevkit/bdk_wallet/releases/tag/v3.0.0-rc.1)
 
**Development:**
- [fees: enable `CBlockPolicyEstimator` return sub 1 sat/vb fee rate estimates](https://github.com/bitcoin/bitcoin/pull/33199)
- [Attack on I2P: Bitcoin nodes not reachable via I2P](https://bnoc.xyz/t/attack-on-i2p-bitcoin-nodes-not-reachable-via-i2p/79/4)
- [Incremental mutation testing in the Bitcoin Core](https://delvingbitcoin.org/t/incremental-mutation-testing-in-the-bitcoin-core/2197)
- [dumptxoutset: enable UTXO snapshot streaming to named pipe](https://github.com/bitcoin/bitcoin/issues/31560)
- [Increase default dbcache from 450 MiB to 1 GiB](https://github.com/bitcoin/bitcoin/issues/34692)
- [More accurate cost model for cluster linearization (SFL)](https://github.com/bitcoin/bitcoin/issues/34616)
 
### Security
- [Binohash](https://robinlinus.com/binohash.pdf) — [Binohash: Transaction Introspection Without Softforks](https://delvingbitcoin.org/t/binohash-transaction-introspection-without-softforks)
- [PQ provers for P2PKH outputs](https://delvingbitcoin.org/t/pq-provers-for-p2pkh-outputs)
- [Boomerang: Bitcoin Cold Storage with Built-In Coercion Resistance](https://delvingbitcoin.org/t/boomerang-bitcoin-cold-storage-with-built-in-coercion-resistance)
- [Protect AES-256 wallet encryption key with secure_allocator](https://github.com/bitcoin/bitcoin/issues/31774)
 
### BIPs & Proposals
- [BIP446 (OP_TEMPLATEHASH) y BIP448 (Taproot-native Rebindable Transactions)](https://github.com/bitcoin/bips/issues/1974) — covenant bundle que habilita LN-Symmetry
- [BIP442 (OP_PAIRCOMMIT) — parte de la propuesta LNHANCE](https://github.com/bitcoin/bips/issues/1699)
- [BIP392: Silent Payment Descriptors](https://github.com/bitcoin/bips/issues/2047)
- [BIP128: Timelock-Recovery storage format](https://github.com/bitcoin/bips/issues/2068)
- [Draft BIP: expanded nVersion nonce space for miners](https://groups.google.com/g/bitcoindev/c/fCfbi8hy-AE)
- [Hourglass V2 Update](https://github.com/cryptoquick/bips/blob/hourglass-v2/bip-hourglass-v2.mediawiki) — [bitcoin-dev discussion](https://groups.google.com/g/bitcoindev/c/0E1UyyQIUA0)
- [Algorithm agility for Bitcoin (Ethan Heilman)](https://groups.google.com/g/bitcoindev/c/7jkVS1K9WLo) — [The limitations of cryptographic agility (Pieter Wuille)](https://groups.google.com/g/bitcoindev/c/O6l3GUvyO7A)
 
### Lightning Network
- [Split splice initiation into two phases](https://github.com/lightningdevkit/rust-lightning/pull/4290)
- [Switch SplicePrototype feature flag to the prod feature bit](https://github.com/lightningdevkit/rust-lightning/pull/4387)
- [LDK #4427: RBF fee bumping of splice funding transactions](https://github.com/lightningdevkit/rust-lightning/issues/4427)
- [Core Lightning #8817: splice interoperability fixes with Eclair](https://github.com/ElementsProject/lightning/issues/8817)
- [LDK #4304: HTLC forwarding refactor for trampoline routing](https://github.com/lightningdevkit/rust-lightning/issues/4304)
- [LND #10089: onion message forwarding (toward BOLT12)](https://github.com/lightningnetwork/lnd/issues/10089)
- [BOLTs #1316: offer_amount must be greater than zero](https://github.com/lightning/bolts/issues/1316)
 
### Layer 2
- [Stateless VTXO Verification: Decoupling Custody from Implementation-Specific Stacks](https://delvingbitcoin.org/t/stateless-vtxo-verification-decoupling-custody-from-implementation-specific-stacks)
- [Ciphera - a Bitcoin ZK Appchain Implementation](https://delvingbitcoin.org/t/ciphera-a-bitcoin-zk-appchain-implementation)
- [Thoughts on L2 Rollups and ZkProofs (Comparison of Citrea vs Stacks vs Rootstock)](https://www.scribd.com/presentation/973037182/Citrea-vs-Stacks-vs-Rootstock)
 
### Wallets
- [Sparrow 2.4.0](https://github.com/sparrowwallet/sparrow/releases/tag/2.4.0) (BIP375 silent payment HW support, Codex32 importer) y [2.4.2](https://github.com/sparrowwallet/sparrow/releases/tag/2.4.2) (v3 transaction support)
- [Cake Wallet adds Lightning support](https://blog.cakewallet.com/our-lightning-journey/) (Breez SDK + Spark)
- [Blockstream Jade adds Lightning via Liquid](https://blog.blockstream.com/jade-lightning-payments-are-here/) (submarine swaps, keys offline)
 
### Tools & Development
- [Mining Pool Behavior during Forks](https://b10c.me/blog/014-mining-pool-behavior-during-forks/)
- [FIBRE network relaunched](https://lclhost.org/blog/fibre-resurrected/) (Bitcoin Core v30 rebase, low-latency block propagation)
- [Bitcoin-tui: terminal interface for Bitcoin Core](https://x.com/_jan__b/status/2031741548098896272)
- [Using AI tooling for code review](https://delvingbitcoin.org/t/using-ai-tooling-for-code-review)
 
### Miscellaneous
- [Proposal: Consensus rule to recover 79,956 BTC stolen from MtGox](https://bitcointalk.org/index.php?topic=5575915)